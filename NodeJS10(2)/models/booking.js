...
class Booking {
    ...
    static fetchAll(id) {
        return db.execute('SELECT * FROM bookings where user_id = ?',[id]);
    }
    ...
    static create(booking) {
        const { customer_name, booking_date, booking_time, total_amount, status, payment_method, duration_minutes , user_id} = booking;
        console.log(booking);
        if (!customer_name || !booking_date || !booking_time || !total_amount || !status || !payment_method || !duration_minutes || !user_id) {
            throw new Error('All fields are required');
        }

        return db.execute(
            'INSERT INTO bookings (customer_name, booking_date, booking_time, total_amount, status, payment_method, duration_minutes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', 
            [customer_name, booking_date, booking_time, total_amount, status, payment_method, duration_minutes, user_id]
        ).catch(err => {
            throw new Error(err);
        });
    }
